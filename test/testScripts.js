...
contract('product',()=>{
    ...
    it('Should mint the token properly', async () => {
        ...
        await productDep.mint("serial","zip",{from: manufacturerAddress,gas:3000000});
    ...
    it('Should NOT mint the token having same serial number', async () => {
        ...
        var manufacturerAddress = await productDep.manufacturer.call();
        await truffleAssert.reverts(
            productDep.mint("serial","zipDifferent",{from: manufacturerAddress,gas:3000000}),
    ...
    it('Should transfer token properly', async () => {
        ...
        var manufacturerAddress = await productDep.manufacturer.call();
        var accounts = await web3.eth.getAccounts();
        await productDep.transfer(accounts[1],1,{from: manufacturerAddress,gas:3000000});
  ...
  //truffleAssert handles the error and passes the test
    it('Should NOT transfer waiting token', async () => {
        ...
        var accounts = await web3.eth.getAccounts();
        await truffleAssert.reverts(
            productDep.transfer(accounts[2],1,{from: manufacturerAddress,gas:3000000}),
    ...
    it('Should approve transfer properly', async () => {
        ...
        var lastOwner = await productDep.getLastOwner(1,{from: accounts[1],gas:3000000});
    ...
    it('Should NOT transfer a token belonging to others', async () => {
        ...
        var manufacturerAddress = await productDep.manufacturer.call();
        var accounts = await web3.eth.getAccounts();
        await productDep.mint("serial1","zip1",{from: manufacturerAddress,gas:3000000});
        await truffleAssert.reverts(
    ...
    it('Consumer can traceback the token owners properly', async () => {
        ...
        var manufacturerAddress = await productDep.manufacturer.call();
        var accounts = await web3.eth.getAccounts();
        await productDep.transfer(accounts[2],1,{from: accounts[1],gas:3000000});
    ...
    it('Owner of the transfer should be able to cancel it before approval', async () => {
        const productDep = await product.deployed();
        var manufacturerAddress = await productDep.manufacturer.call();
        await productDep.mint("serial3","zip3",{from: manufacturerAddress,gas:3000000});
        var accounts = await web3.eth.getAccounts();
        await productDep.transfer(accounts[3],3,{from: manufacturerAddress,gas:3000000});
        var expectedBefore = await productDep.waitingtransfers.call(3);
        
        assert(expectedBefore === accounts[3],"1");
        await productDep.cancelTransfer(3,{from: manufacturerAddress,gas:3000000});
    ...
    it('Other people who are not owner of the transfer should NOT be able to cancel it', async () => {
        const productDep = await product.deployed();
        var accounts = await web3.eth.getAccounts();
        await productDep.transfer(accounts[1],3,{from: accounts[3],gas:3000000});
